...
async function rejects(name, args, needle) {
  ...
  if (needle && !text.includes(needle)) throw new Error(`${name}: expected rejection to mention "${needle}", got: ${text}`);
  pass += 1;
...
await step('cluster and node reads', async () => {
  await ok('pve_list_nodes', {}, (d) => (Array.isArray(d) && d[0]?.node === N ? null : 'expected node pve'));
...
await step('storage and network reads', async () => {
  await ok('pve_list_storage', { node: N }, (d) => (d.some((s) => s.storage === 'local') ? null : 'no local storage'));
  await ok('pve_storage_content', { node: N, storage: 'local', content: 'iso' }, (d) => (d.length ? null : 'no iso content'));
  await ok('pve_storage_content', { node: N, storage: 'local', content: 'vztmpl' });
  await ok('pve_list_network', { node: N }, (d) => (d.some((n) => n.iface === 'vmbr0') ? null : 'no vmbr0'));
});
...
await step('VM create and inspect', async () => {
  ...
  await ok('pve_vm_status', { node: N, vmid: VM }, (d) => (d.status === 'stopped' ? null : 'new VM should be stopped'));
...
await step('VM lifecycle and reconfigure', async () => {
  ...
  await ok('pve_vm_status', { node: N, vmid: VM }, (d) => (d.status === 'running' ? null : 'VM should be running after start'));
  await ok('pve_vm_set_config', { node: N, vmid: VM, memory: 2048 });
  await ok('pve_vm_config', { node: N, vmid: VM }, (d) => (Number(d.memory) === 2048 ? null : 'memory not updated'));
  await ok('pve_vm_reboot', { node: N, vmid: VM });
});

await step('snapshots', async () => {
  await ok('pve_snapshot_create', { node: N, vmid: VM, type: 'qemu', snapname: 'base', description: 'pre-change' });
  await ok('pve_list_snapshots', { node: N, vmid: VM, type: 'qemu' }, (d) => (d.some((s) => s.name === 'base') ? null : 'snapshot not listed'));
  ...
  await ok('pve_list_snapshots', { node: N, vmid: VM, type: 'qemu' }, (d) => (d.some((s) => s.name === 'base') ? 'snapshot not deleted' : null));
});

await step('clone and migrate', async () => {
  await ok('pve_vm_clone', { node: N, vmid: VM, newid: VM_CLONE, name: 'sweep-clone', full: true });
  await ok('pve_list_vms', { node: N }, (d) => (d.some((v) => v.vmid === VM_CLONE) ? null : 'clone not present'));
  await ok('pve_vm_migrate', { node: N, vmid: VM_CLONE, target: N, online: false });
});

await step('backup, restore', async () => {
  await ok('pve_backup_now', { node: N, vmid: VM, storage: 'local', mode: 'snapshot', compress: 'zstd' });
  const backups = await ok('pve_list_backups', { node: N, storage: 'local' }, (d) => (d.some((b) => b.vmid === VM) ? null : 'backup not listed'));
  ...
  await ok('pve_restore', { node: N, vmid: VM, type: 'qemu', archive, storage: 'local-lvm', confirm: true, confirmVmid: VM });
  await ok('pve_vm_status', { node: N, vmid: VM }, (d) => (d.vmid === VM ? null : 'restore did not recreate VM'));
});
...
await step('storage image download', async () => {
  ...
  await ok('pve_storage_content', { node: N, storage: 'local', content: 'iso' }, (d) => (d.some((c) => c.volid.endsWith('alpine-3.20.iso')) ? null : 'downloaded iso not present'));
});

await step('network changes', async () => {
  await ok('pve_create_bridge', { node: N, iface: 'vmbr1', autostart: true, cidr: '10.10.0.1/24' });
  await ok('pve_list_network', { node: N }, (d) => (d.some((n) => n.iface === 'vmbr1') ? null : 'vmbr1 not created'));
  await ok('pve_apply_network', { node: N, confirm: true });
});

await step('scheduled backup', async () => {
  await ok('pve_schedule_backup', { schedule: 'sat 02:00', storage: 'local', all: true, mode: 'snapshot', compress: 'zstd' });
});
...
await step('safety gates (must reject)', async () => {
  ...
  await rejects('pve_vm_delete', { node: N, vmid: 9999, confirm: true, confirmVmid: 1 }, 'mismatch');
  await rejects('pve_snapshot_rollback', { node: N, vmid: 9999, type: 'qemu', snapname: 'x', confirmName: 'x' }, 'confirm=true');
...
await step('error mapping (bad auth)', async () => {
  ...
  try {
    ...
    if (!/authentication failed|401/i.test(err.message)) throw new Error(`unexpected error: ${err.message}`);
    pass += 1;
