...
const content = {
  local: [
    {
      volid: 'local:iso/debian-12.7.0-amd64-netinst.iso',
      content: 'iso',
      format: 'iso',
      size: 6.5e8,
    },
...
const routes = [
  ...
  [
    ...
    (_p, q) => {
      ...
      return q.type
        ? all.filter((r) =>
            q.type === 'vm' ? r.type === 'qemu' || r.type === 'lxc' : r.type === q.type,
          )
        : all;
    },
  ],
  ...
  [
    ...
    (p) => {
      ...
      return {
        ...
        name: v.name,
        cpus: v.config.cores,
        maxmem: v.config.memory * 1024 * 1024,
        uptime: v.status === 'running' ? 60 : 0,
      };
    },
  ],
  ...
  [
    ...
    (p) => {
      ...
      return {
        ...
        name: c.name,
        cpus: c.config.cores,
        maxmem: c.config.memory * 1024 * 1024,
      };
    },
  ],
  ...
  [
    'GET',
    /^\/nodes\/([^/]+)\/network$/,
    (_p, q) => (q.type ? networks.filter((n) => n.type === q.type) : networks),
  ],

  ['POST', /^\/nodes\/([^/]+)\/qemu$/, (_p, _q, b) => createOrRestore('qemu', b)],
  [
    'POST',
    /^\/nodes\/([^/]+)\/qemu\/(\d+)\/status\/(start|stop|shutdown|reboot|reset)$/,
    (p) => lifecycle('qemu', p[2], p[3]),
  ],
  ...
  [
    'POST',
    /^\/nodes\/([^/]+)\/lxc\/(\d+)\/status\/(start|stop|shutdown|reboot)$/,
    (p) => lifecycle('lxc', p[2], p[3]),
  ],
  ...
  [
    'POST',
    /^\/nodes\/([^/]+)\/(qemu|lxc)\/(\d+)\/snapshot\/([^/]+)\/rollback$/,
    (p) => newUpid('qmrollback', p[3]),
  ],
  [
    'DELETE',
    /^\/nodes\/([^/]+)\/(qemu|lxc)\/(\d+)\/snapshot\/([^/]+)$/,
  ...
  [
    ...
    (p, _q, b) => {
      (content[p[2]] ??= []).push({
        volid: `${p[2]}:${b.content}/${b.filename}`,
        content: b.content,
        size: 1e8,
      });
  ...
  [
    ...
    (_p, _q, b) => {
      networks.push({
        iface: b.iface,
        type: b.type,
        method: b.cidr ? 'static' : 'manual',
        cidr: b.cidr,
        active: 0,
        autostart: b.autostart === '1' ? 1 : 0,
      });
      return null;
    },
  ],
  ...
  [
    ...
    (_p, _q, b) => {
      ...
      (content[b.storage] ??= []).push({
        volid: `${b.storage}:backup/vzdump-qemu-${b.vmid}-${ts}.vma.zst`,
        content: 'backup',
        vmid: Number(b.vmid),
        size: 5e8,
      });
...
function createOrRestore(kind, b) {
  ...
  return newUpid(
    kind === 'qemu' ? (isRestore ? 'qmrestore' : 'qmcreate') : isRestore ? 'vzrestore' : 'vzcreate',
    vmid,
  );
}
...
function clone(kind, vmid, b) {
  ...
  map.set(newid, {
    vmid: newid,
    name: b.name ?? b.hostname ?? `clone-${newid}`,
    status: 'stopped',
    config: { ...src.config, vmid: newid },
  });
