...
export function registerSnapshotTools(reg) {
    reg.register({
        ...
        return ctx.client.get(`/nodes/${node}/${type}/${vmid}/snapshot`);
    });
    reg.register({
    ...
    reg.register({
        ...
        description: 'Roll a guest back to a snapshot. The current state is discarded. Requires confirm=true ' +
        ...
        const data = await ctx.client.post(`/nodes/${args.node}/${args.type}/${args.vmid}/snapshot/${args.snapname}/rollback`);
        return settleTask(ctx, data, { action: 'snapshot_rollback', snapname: args.snapname });
    });
    reg.register({
        name: 'pve_snapshot_delete',
        title: 'Delete snapshot',
        description: 'Delete a snapshot. Requires confirm=true and confirmName set to the snapshot name.',
        schema: {
            node: nodeField,
            vmid: vmidField,
            type: guestType,
        ...
        requireMatchingId(args.confirmName, args.snapname, `snapshot "${args.snapname}"`);
        const data = await ctx.client.delete(`/nodes/${args.node}/${args.type}/${args.vmid}/snapshot/${args.snapname}`);
