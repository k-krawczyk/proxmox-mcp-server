...
export function registerBackupTools(reg) {
    reg.register({
    ...
    reg.register({
        name: 'pve_backup_now',
        title: 'Backup now',
        description: 'Run a one-off vzdump backup of a guest to a storage. Returns the task outcome when the ' +
            'backup finishes.',
        schema: {
            node: nodeField,
            vmid: vmidField,
        ...
        if (args.notes)
            params['notes-template'] = args.notes;
    ...
    reg.register({
        ...
        description: 'Restore a backup archive into a guest id. If the target id already exists its contents ' +
        ...
        schema: {
            ...
            archive: z
                .string()
                .min(1)
                .describe('Backup volume id, e.g. "local:backup/vzdump-qemu-100-2024_01_01-00_00_00.vma.zst"'),
        ...
        const params = { vmid: args.vmid, force: args.force, storage: args.storage };
    ...
    reg.register({
        ...
        description: 'Create a recurring cluster backup job. Schedule uses systemd calendar syntax, e.g. ' +
