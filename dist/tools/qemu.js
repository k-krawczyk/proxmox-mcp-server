...
export function registerQemuTools(reg) {
    reg.register({
        name: 'pve_list_vms',
        title: 'List VMs',
        description: 'List QEMU VMs. With a node, lists that node directly; without one, returns every VM in ' +
        ...
        return ctx.client.get('/cluster/resources', { params: { type: 'vm' } });
    });
    reg.register({
        name: 'pve_vm_status',
        title: 'VM status',
        ...
        return ctx.client.get(`/nodes/${node}/qemu/${vmid}/status/current`);
    });
    reg.register({
        name: 'pve_vm_config',
        title: 'VM config',
    ...
    reg.register({
        name: 'pve_vm_create',
        title: 'Create VM',
        description: 'Create a new QEMU VM. Provide structured disk/net/iso, or use the raw passthrough for ' +
    ...
    reg.register({
        name: 'pve_vm_clone',
        title: 'Clone VM',
        description: 'Clone a VM or template into a new VMID. Linked clone by default; set full=true for an ' +
    ...
    reg.register({
        ...
        description: 'Change VM resources or settings. Pass only the fields to change; everything else is ' +
    ...
    reg.register({
        name: 'pve_vm_delete',
        title: 'Delete VM',
        description: 'Permanently delete a VM and its disks. Requires confirm=true and confirmVmid set to the ' +
...
function registerLifecycle(reg, op, destructive, description) {
    reg.register({
        name: `pve_vm_${op}`,
        title: `VM ${op}`,
        description,
        ...
        guardTarget(ctx, args.node, args.vmid);
        if (destructive) {
            requireConfirm(args.confirm, `${op} VM ${args.vmid}`);
        }
