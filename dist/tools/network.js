...
export function registerNetworkTools(reg) {
    reg.register({
        name: 'pve_list_network',
        title: 'List network interfaces',
        description: 'List network interfaces on a node (bridges, bonds, physical NICs, VLANs). Pending, ' +
        ...
        return ctx.client.get(`/nodes/${node}/network`, {
    ...
    reg.register({
        ...
        title: 'Create bridge',
        description: 'Create a Linux bridge on a node. The change is staged as pending; call pve_apply_network ' +
            'to activate it.',
        schema: {
            node: nodeField,
            iface: z
    ...
    reg.register({
        ...
        description: 'Apply all pending network changes on a node. This reloads networking and can drop ' +
