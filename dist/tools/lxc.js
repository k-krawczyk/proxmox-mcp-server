...
export function registerLxcTools(reg) {
    reg.register({
        name: 'pve_list_containers',
        title: 'List containers',
        description: 'List LXC containers. With a node, lists that node directly; without one, returns every ' +
    ...
    reg.register({
        ...
        return ctx.client.get(`/nodes/${node}/lxc/${vmid}/status/current`);
    });
    reg.register({
        name: 'pve_lxc_config',
    ...
    reg.register({
        ...
        description: 'Create a new LXC container from a template. The template must already exist on storage ' +
        ...
        schema: {
            ...
            ostemplate: z
                .string()
                .min(1)
                .describe('Template volume id, e.g. "local:vztmpl/debian-12-standard_12.7-1_amd64.tar.zst"'),
    ...
    reg.register({
        ...
        description: 'Permanently delete a container and its rootfs. Requires confirm=true and confirmVmid set ' +
...
function registerLifecycle(reg, op, destructive, description) {
    reg.register({
        ...
        guardTarget(ctx, args.node, args.vmid);
        if (destructive) {
            requireConfirm(args.confirm, `${op} container ${args.vmid}`);
        }
